...
global passengers_refused
global passengers_misplaced
global c
...
def get_seat_availability_threes(rows, num_rows):
    seat_availability_threes = dict()

    for row in rows:
        if row.SeatsAvailable >= 3 and row.SeatsAvailable < num_rows:
            seat_availability_threes[row.RowNumber] = row.SeatsAvailable


def get_seat_availability_row_size(rows, num_rows):
    seat_availability_num_rows = dict()

    for row in rows:
        if row.SeatsAvailable == num_rows:
            seat_availability_num_rows[row.RowNumber] = row.SeatsAvailable
...
def  split_booking_between_twos_and_threes(booking_size, rows, num_seats_per_row, passenger_name):
    ...
    seat_availability_threes = get_seat_availability_threes(rows, num_seats_per_row)
...
def assign_seats_where_booking_size_exceeds_row_size(booking_size, rows, num_seats_per_row, passenger_name):
    ...
    booking_successful = split_booking_between_twos_threes_and_row_size(booking_size, rows, num_seats_per_row, passenger_name)
...
def  split_booking_between_twos_threes_and_row_size(booking_size, rows, num_seats_per_row, passenger_name):
...
if __name__ == "__main__":
    ...
    for booking in bookings_list:
        ...
        if seats_available == True:
           assign_seating(booking_size, passenger_name, all_rows, num_cols)
           total_empties = total_empties - booking_size
        else:
            passengers_refused += booking_size


    write_passenger_stats_to_db()
