...
class Row:
    ...
    def AddSeatToRow(self, seat):
        # This method adds a new seat to the row in question
...
def edit_seating_plan_for_testing(num_rows, seat_arrangement):
    # Deletes table data and re-creates database to reflect a new plane structure being passed in for testing.

    # This command updates the rows_cols table to reflect the new seating arrangement
    sql_text = "update rows_cols set seats =  '" + seat_arrangement + "', nrows = " + str(num_rows)
    c.execute(sql_text)

    # This command deletes all current data from the seating table
    sql_text = "delete from seating"
    c.execute(sql_text)

    split_string = list(seat_arrangement)

    # Inserts a new seat into the table for every row and column
    for i in range(1, num_rows+1): # i = row number
        for j in split_string: # j = column title
            sql_text = "insert into seating (name, row, seat) values ('', "+ str(i) + ", '"+ str(j)+"')"
            c.execute(sql_text)
...
def search_and_assign_most_suitable_seats(rows, booking_size, passenger_name):
    ...
    # searching for a row with the number of seats + 2 to leave space for a pair to also be accommodated
    for row in rows:
...
def assign_seats_where_booking_size_less_than_or_equal_to_row_size(booking_size, rows, num_seats_per_row, passenger_name):
    ...
    booking_accommodated_altogether = search_and_assign_most_suitable_seats(rows, booking_size, passenger_name)

    if booking_accommodated_altogether == True:
...
def split_booking_across_rows(booking_size, rows, num_seats_per_row, passenger_name):
    ...
    while seats_left_to_fill > 0:
        booking_for_subgroup_accommodated = search_and_assign_most_suitable_seats(rows, subgroup_size, passenger_name)

        if booking_for_subgroup_accommodated == True:
...
def fill_seats_with_2_and_3_configurations(rows,  passenger_name, num_2_bookings, num_3_bookings):
    ...
    # the number of groups of 2 and groups of 3 are passed in
    # the relevant number of seats are assigned to each group of 2 and group of 3

    for i in range(0, num_2_bookings):
        booking_accommodated = search_and_assign_most_suitable_seats(rows, 2, passenger_name)
        if booking_accommodated == False: raise Exception("Booking was not accommodated despite passing sufficient space checks.")

    for i in range(0, num_3_bookings):
        booking_accommodated = search_and_assign_most_suitable_seats(rows, 3, passenger_name)
        if booking_accommodated == False: raise Exception("Booking was not accommodated despite passing sufficient space checks.")
...
def run_bookings_assignment(bookings_file_path):
    ...
    passengers_seated = 0 # this metric is for testing purposes only
    ...
    for booking in bookings_list:
        ...
        if booking_size > 0: # probably unnecessary; more of an error check
            ...
            # checks to see if there is availability for the booking
            ...
            if seats_available == True:
                ...
                passengers_seated += booking_size
            else:
    ...
    # these are for clarity on the console and helpful for testing purposes
    ...
    print("Total Passengers Seated: " + str(passengers_seated))
    print("")

    print("Bookings complete.")

def test(bookings_file_path):
    # This is a method which has been used to test the code along with a number of different plane configurations.
    # It iterates over a row size and a plane structure, creating this new plane structure in the database
    # It then assigns the seats as normal using the exact same code with the new plane structure

    for i in range(1,40):
        # number of rows - used range 1-40 but in reality this could be higher. Probably unnecessary to go any higher.
        seat_string = ""
        for j in ("A","B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L"):
            # number of columns - used range 1-12 seats per row. Again, this could be made higher but it is probably unnecessary
            seat_string += j
            edit_seating_plan_for_testing(i, seat_string) # edits database to reflect new plane structure being tested
            run_bookings_assignment(bookings_file_path) # runs assignment code with new plane structure
